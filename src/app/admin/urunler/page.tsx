...
import { defaultProducts, defaultImages } from '@/data/defaults';
import CustomSelect from '@/components/CustomSelect';
import { compressImage } from '@/utils/image';

const categories = ['PDF Rehberler', 'Video Eğitimler', 'Yayın Kayıtları'];
const icons = [
  { name: 'Belge', icon: 'HiOutlineDocumentText' },
  { name: 'Oynat', icon: 'HiOutlinePlay' },
  { name: 'Kitap', icon: 'HiOutlineBookOpen' },
];
const badges = ['Yok', 'Yeni', 'Çok Satan'];

export default function AdminUrunler() {
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [price, setPrice] = useState('');
  const [category, setCategory] = useState('PDF Rehberler');
  const [icon, setIcon] = useState('HiOutlineDocumentText');
  const [badge, setBadge] = useState('Yok');
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      localStorage.setItem('custom_products', JSON.stringify(defaultProducts));
    }
  }, []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const productData = {
      ...
      category,
      icon,
      badge: badge === 'Yok' ? null : badge,
      gradient: editingIndex !== null ? products[editingIndex].gradient : 'from-gold/8 to-beige/20',
      iconGrad: editingIndex !== null ? products[editingIndex].iconGrad : 'from-gold to-gold-dark',
      image: image,
    };
    ...
    setDescription('');
    setPrice('');
    setCategory('PDF Rehberler');
    setIcon('HiOutlineDocumentText');
    setBadge('Yok');
  };
  ...
  const handleEdit = (index: number) => {
    ...
    setBadge(product.badge || 'Yok');
    setImage(product.image || defaultImages[0].url);
  ...
  return (
    <>
      <header className="mb-12">
        <h2 className="font-serif text-4xl text-wine mb-2">Ürünler Yönetimi</h2>
        <p className="text-taupe/50 text-sm tracking-widest uppercase">Yeni ürün ekleyin veya mevcutları silin.</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="premium-card p-8 bg-white/60 backdrop-blur-2xl">
            <h3 className="font-serif text-2xl text-wine mb-6">{editingIndex !== null ? 'Ürünü Düzenle' : 'Yeni Ürün Ekle'}</h3>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  placeholder="Örn: Ruhsal Yolculuk"
                  required
                />
              </div>
              <div>
              ...
              <div>
                <label className="block text-[0.7rem] uppercase tracking-widest font-bold text-gold mb-2">Fiyat (₺)</label>
                <input
                  type="text"
                  value={price}
              ...
              <div>
                <label className="block text-[0.7rem] uppercase tracking-widest font-bold text-gold mb-4">Ürün Görseli</label>
              ...
              <div>
                ...
                <CustomSelect
                  value={icon}
                  onChange={setIcon}
                  options={icons.map((item) => ({ name: item.name, value: item.icon }))}
                />
              </div>
              <div>
                <label className="block text-[0.7rem] uppercase tracking-widest font-bold text-gold mb-2">Rozet</label>
                <CustomSelect
                  value={badge}
                  onChange={setBadge}
                  options={badges}
        ...
        <div className="lg:col-span-2">
          <div className="premium-card p-8 bg-white/40">
            <h3 className="font-serif text-2xl text-wine mb-6">Eklenen Ürünler</h3>
            {products.length === 0 ? (
              <p className="text-taupe/60 text-center py-12">Henüz yeni bir ürün eklenmemiş.</p>
            ) : (
              <div className="space-y-4">
                {products.map((product, index) => (
                  <div key={index} className="flex items-center justify-between p-6 bg-white/80 rounded-2xl border border-gold/5">
                    <div>
                      <h4 className="font-serif text-xl text-wine">{product.title}</h4>
                      <p className="text-sm text-taupe/60 max-w-md">{product.description}</p>
                      <div className="flex gap-2 mt-2">
                        <span className="text-xs bg-gold/10 text-gold-dark px-2 py-1 rounded-full">{product.category}</span>
                        <span className="text-xs bg-burgundy/10 text-burgundy px-2 py-1 rounded-full">{product.price}</span>
                        {product.badge && <span className="text-xs bg-wine text-ivory px-2 py-1 rounded-full">{product.badge}</span>}
                      </div>
                    </div>
